$(document).ready(function(){  
    ...
    var abrir_menu_portada = setTimeout(function(){
  		$(".menu-open-button").click();
	}, 3000);

    $( "body" ).on( "click", ".menu-open-button", function(e){

        clearTimeout(abrir_menu_portada);

    });

    $( "body" ).on( "click", ".imagen-principal-excursiones,.reserva-sultan-popup", function(e){
        var id_excursion = $(this).attr('id');
        $(id_excursion).css('display','block');
        $('#page-content-scroll').css('overflow','hidden');
        $('.content-cerrar').css('display','block');
        $('.input-reserva').val('');
        var swiper = new Swiper(id_excursion + ' > .swiper-container');
    });

    $( "body" ).on( "click", ".btn-cerrar-popup", function(e){
        $('.content-excursiones,.content-cerrar').css('display','none');
        $('#page-content-scroll').css('overflow','auto');
    });

    /*$( "body" ).on( "click", ".img-encuesta", function(e){
        console.log('holaa');
       $(this).parent().parent().children('.content-ico-encuesta').children('img').removeClass('encuesta-activa');
       $(this).addClass('encuesta-activa');
    });*/

    $( "body" ).on( "click", ".img-encuesta", function(e){
        if($(this).parent().hasClass('iconos-centrados-encuesta')){
            $(this).parent().parent().children('.content-ico-encuesta').removeClass('iconos-centrados-encuesta');
            $(this).parent().removeClass('icono-activo-index');
        }else{
            $(this).parent().parent().children('.content-ico-encuesta').addClass('iconos-centrados-encuesta');
            $(this).parent().addClass('icono-activo-index');
        }

    });

    $( "body" ).on( "click", ".btn-enviar-reserva", function(e){

        if($(this).hasClass('btn-enviar-encuesta')){
            $('.reserva-registrada').css('display','block');
            $('.reserva-registrada').fadeOut(3000);
            $('.img-encuesta').removeClass('encuesta-activa');

            $('.content-ico-encuesta').removeClass('iconos-centrados-encuesta');
            $('.content-ico-encuesta').removeClass('icono-activo-index');
