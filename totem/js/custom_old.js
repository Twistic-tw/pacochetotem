...
var delayScreenSaverDeactivationDelay = 30000;   //iundica cuanto tarda en desactivarse el salvapantalla una vez activado
...
var reloadIntervalTime = 30*60*1000;             //30minutos
...
var diasSemanasIdiomas = {
    ...
    en: {0: "Sunday", 1: "Monday", 2: "Thuesday", 3: "Wednesday", 4: "Thursday", 5: "Friday", 6: "Saturday"},
    de: {0: "", 1: "", 2: "", 3: "", 4: "", 5: "", 6: ""},
...
//    });
    ...
    // cargamos mediante ajax los detalles relacionados con la informacion meteorologica y con los detalles de la agenda
    setTimeout(function() {

       // generarGraficaMeteorologica();

        //Llamar a la funcion que coje los eventos los tres dias
         //       getListadoEventosAgenda3(true, function() {
            //para realizar la peticion de cargar las actividades del dia actual.
            
       // });



        getListadoEventosAgenda(true, function() {
            //para realizar la peticion de cargar las actividades del dia actual.
    ...
    $("#agendaCalendario").datepicker({
        ...
        beforeShowDay: function(date) {
            ...
            if (fechasActivas_tabActive && fechasActivas_tabActive[t]) {
                obj[0] = true;
        ...
        if (!$(event.target).hasClass("icon-svg")) {
            ...
            $(this).parents(".actividadesList").popup({
                ...
                renderCallback: function(){
                    ...
                    $(".agenda_personalizada .eventoDetailContent").append('<div class="rating_informacion">' +

                        '<div class="rating_info">' +
                            '<div class="rating_info_habitacion">' +
                                '<div class="rating_info_icon icono_habitacion"></div>'  +
                                '<input placeholder="'+$(obj).find(".lang_habitacion").html()+'" type="text"/>'  +
                            '</div>' +  
                            '<div class="rating_info_apellidos">' +
                                '<div class="rating_info_icon icono_lapiz"></div>'  +
                                '<input placeholder="'+$(obj).find(".lang_apellidos").html()+'" type="text"/>'  + 
                            '</div>' +
                        '</div>' +


                        '<div class="rating_votacion">' +
                            '<div class="rating_votacion_grupo">' +
                                '<div class="rating_info_icon icono_tick"></div>'  +
                                '<div class="rating_votacion_texto">'+$(obj).find(".lang_enviar").html()+'</div>'  +
                            '</div>' +
                            '<div class="rating_votacion_grupo">' +
                                '<div class="rating_info_icon icono_cross"></div>'  +
                                '<div class="rating_votacion_texto">'+$(obj).find(".lang_nogracias").html()+'</div>'  + 
                            '</div>' + 
                        '</div>' + 

                        '</div>');
                        /////////////////////////////////////////////////////////////////////////////////////////////////
                        //Movidas para la votacion comentadas en la version de RIU que no quiere valoraciones de nada

                          //  $(".agenda_personalizada .eventoDetailContent").append('<div class="rating_informacion_gracias">'+$(obj).find(".lang_gracias").html()+'</div>');

                         //   initializeKeyboardListeners();


                           // $(".agenda_personalizada .eventoDetailContent").append('<div class="agenda_estrellas"><div class="valorar_agenda">'+$(obj).find(".lang_valorar").html()+'</div><div><input class="hover-star" type="radio" name="test-3B-rating-1" onclick="toggle(this)" value="1" title="Very poor"><input class="hover-star" type="radio" name="test-3B-rating-1" onclick="toggle(this)" value="2" title="Poor"><input class="hover-star" type="radio" name="test-3B-rating-1" onclick="toggle(this)" value="3" title="OK"><input class="hover-star" type="radio" name="test-3B-rating-1" onclick="toggle(this)" value="4" title="Good"><input class="hover-star" type="radio" name="test-3B-rating-1" onclick="toggle(this)" value="5" title="Very Good"></div></div>');

                          //  inicializa_listener_rating();


                            /*
                            $(".rating_votacion_grupo").click(function(event){

                               $(".rating_informacion").fadeOut();
                               $(".rating_informacion_gracias").fadeIn();
...
$.fn.showSimilarEvents = function() {
    ...
    $(".actividadesList").append("<li class='showActividades center'>Mostrar todos</li>");
    ...
    if (responseDatos.error) {
        ...
        $("form#enviarPostal").ajaxForm({
            ...
            beforeSubmit: function(){
                ...
                html2canvas( $("#postal")[0], {
                    onrendered: function(canvas){
                        $("form#enviarPostal").find("input#canvasDeLaPostal").val(canvas.toDataURL("image/png"));
                        lockForCanvas = true;
            ...
            success: function(){
                ...
                lockForCanvas = false;
                removeOverlayContentLoadingGif();
                $("form#enviarPostal").after("<h5 class='mensaje_respuesta text textAlign-center'>Muchas gracias por utilizar este servicio.</h5>");
                $("#email_postal").val("");
            ...
            //recibo la respuesta del servidor.
                lockForCanvas = false;
                removeOverlayContentLoadingGif();
                $("form#enviarPostal").after("<h5 class='mensaje_respuesta text textAlign-center'>No se ha podido enviar el mensaje</h5>");
                $("#email_postal").val("");
