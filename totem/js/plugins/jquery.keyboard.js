...
// allow navigation keys to work - Chrome doesn't fire a keypress event (8 = bksp)
                    ...
                    if ($.inArray(k, base.acceptedKeys) === -1) {
                        e.preventDefault();
                    } // quick key check
...
// Allow mousewheel to scroll through other key sets of the same key
                ...
                if (base.wheel) {
                    ...
                    if (key.curNum > txt.length - 1) {
                        key.curNum = 0;
                    }
                    if (key.curNum < 0) {
                        key.curNum = txt.length - 1;
                    }
                    key.layers = txt;
...
// keep keyboard open if alwaysOpen or stayOpen is true - fixes mutliple always open keyboards or single stay open keyboard
            if (!base.isVisible() || (o.alwaysOpen && !cur) || (!o.alwaysOpen && o.stayOpen && cur && !base.isVisible())) {
                return;
            }
            ...
            $.each($.keyboard.layouts[o.layout], function(set, keySet) {
                if (set !== "") {
                    ...
                    newSet = $('<div />')
                            ...
                            .attr('id', "keyboard-container")
                            .addClass('marco-keyboard ui-keyboard-keyset ui-keyboard-keyset-' + set)
