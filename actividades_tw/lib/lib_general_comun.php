...
function get_colores_categorias()
{
    ...
    $queryString = "SELECT *
                    FROM " . $_SESSION['bbdd_comun'] . ".eventos_categorias
                    WHERE activo = 1";
...
function get_categorias($dias = 1)
{
    ...
    $queryString = "SELECT t7.id_cat, t7.identificador, t7.color, t8.nombre as title, t8.descripcion as content, t8.archivo
                    FROM " . $_SESSION['bbdd_comun'] . ".eventos AS t1 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_tiene_idiomas AS t2 ON t1.id_evento = t2.id_evento 
                    INNER JOIN " . $_SESSION['bbdd_normal'] . ".eventos_ocurrencias AS t3 ON t1.id_evento = t3.id_evento 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_tiene_lugares AS t4 ON t1.id_evento = t4.id_evento 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_lugares AS t5 ON t4.id_lugar = t5.id_lugar 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_tiene_categorias AS t6 ON t1.id_evento = t6.id_evento 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_categorias AS t7 ON t7.id_cat = t6.id_cat 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_categorias_tiene_idiomas AS t8 ON t7.id_cat = t8.id_cat 
                    WHERE t1.estado > 0 
                    AND t2.id_idioma = '" . $id_idioma . "' 
                    AND t8.id_idioma = '" . $id_idioma . "' 
                    AND t3.fecha BETWEEN '" . $fecha_inicial . "' AND '" . $fecha_final . "' 
                    GROUP BY t7.id_cat
                    ORDER BY t7.orden, t3.hora_ini ASC";
...
function get_actividades3($dias = 1, $ids_categorias = null)
{
    ...
    if (isset($ids_categorias) && $ids_categorias != null) {
        $filtro_categoria = ' AND t7.id_cat IN (' . $ids_categorias . ') ';
    }
    ...
    $queryString = "SELECT t2.descripcion as content, t1.id_evento, t2.titulo as title, t1.image as foto_evento, t1.video as video_evento, t7.id_cat, t7.identificador AS identificador_lugar, t7.color, t7.imagen as foto_evento, t3.fecha, t3.hora_ini, t3.hora_fin  
                    FROM " . $_SESSION['bbdd_comun'] . ".eventos AS t1 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_tiene_idiomas AS t2 ON t1.id_evento = t2.id_evento 
                    INNER JOIN " . $_SESSION['bbdd_normal'] . ".eventos_ocurrencias AS t3 ON t1.id_evento = t3.id_evento 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_tiene_lugares AS t4 ON t1.id_evento = t4.id_evento 
...
function get_actividades_hoy($dias = 1)
{
    ...
    $queryString = "SELECT t2.descripcion as content, t1.id_evento, t2.titulo as title, t1.image as foto_evento, t1.video as video_evento, t7.id_cat, t7.identificador, t7.color, t7.imagen, t3.fecha, t3.hora_ini, t3.hora_fin  
                    FROM " . $_SESSION['bbdd_comun'] . ".eventos AS t1 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_tiene_idiomas AS t2 ON t1.id_evento = t2.id_evento 
                    INNER JOIN " . $_SESSION['bbdd_normal'] . ".eventos_ocurrencias AS t3 ON t1.id_evento = t3.id_evento 
                    INNER JOIN " . $_SESSION['bbdd_comun'] . ".eventos_tiene_lugares AS t4 ON t1.id_evento = t4.id_evento 
